...
# First of all, check if any bibliography is requested. If not, you're done.
      ...
      for biblio in bibList
# The bibliography element will have different attributes
        file = $(biblio).attr("data-bibliography-file") or "bibliography.bib" # The file containing all entries
        fileFormat = $(biblio).attr("data-bibliography-file-format") or "bibtex" # The file format (bibtex, bibjson, etc.)
        format = $(biblio).attr("data-bibliography-style") or "plain" # The format to use for citations
        identifier = $(biblio).attr("data-bibliography-identifier") or "" # Identifier of the bibliography for multiple bibliographies
...
# This maybe can be done in a better way. Feel free to propose a better architecture.
          $(biblio).append(formatter.createItems(usedEntries))
